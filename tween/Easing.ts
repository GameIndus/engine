    ...
    export function apply(easing: Easing, v: number): number {
        switch (easing) {
            ...
            case Easing.BACK_IN_OUT:
                ...
                else return 0.5 * ((v -= 2) * v * ((s + 1) * v + s) + 2);
            ...
            case Easing.BOUNCE_IN_OUT:
                if (v < 0.5) return Easing.apply(Easing.BOUNCE_IN, v * 2) * 0.5;
                else return Easing.apply(Easing.BOUNCE_OUT, v * 2 - 1) * 0.5 + 0.5;
            ...
            case Easing.CIRCULAR_IN_OUT:
                ...
                else return 0.5 * (Math.sqrt(1 - (v -= 2) * v) + 1);
            ...
            case Easing.ELASTIC_IN_OUT:
                ...
                if (v < 1) return -0.5 * Math.pow(2, 10 * (v - 1)) * Math.sin((v - 1.1) * 5 * Math.PI);
                else return 0.5 * Math.pow(2, -10 * (v - 1)) * Math.sin((v - 1.1) * 5 * Math.PI) + 1;
            ...
            case Easing.EXPONENTIAL_IN_OUT:
                ...
                else return 0.5 * (-Math.pow(2, -10 * (v - 1)) + 2);
